...
export default function PrivacyPage({ legalInitial, isDraftMode = false }) {
  const { data } = useQuery(LEGAL_QUERY, { pageKey: 'privacy' }, { initial: legalInitial });
  const sections = data?.sections ?? [];
  const ids = sections.map(s => s.id);
  const activeId = useActiveSection(ids);
  const dateLabel = formatDate(data?.lastUpdated) ?? 'April 12, 2026';
