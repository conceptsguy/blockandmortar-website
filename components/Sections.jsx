/* global React */
// Sections.jsx — Nav, Hero shell, TrustedBy, Steps, Bento, Verticals, CTA, Footer
const { useEffect } = React;

function FileStackAnim() {
  // Files fly in from the left, stack up neatly in the right ~150px of the
  // container, a grouping frame reveals around them, then the whole group
  // scales and fades as if being submitted into the archive.
...
function GisMapAnim() {
  // Real Kansas City map (CartoDB Positron tiles, no-labels) rendered as a
  // 4×2 mosaic, pushed to grayscale + cream filter to read as monochrome.
  // A teardrop pin drops into the center with a pulsing accuracy ring.
  const [tick, setTick] = React.useState(0);
  React.useEffect(() => {
  ...
  // Tiles centered on downtown Kansas City at zoom 14.
  // KC: 39.0997° N, -94.5786° W → tile (3890, 6268) at z14.
  const z = 14;
  const baseX = 3888;
  const baseY = 6267;
  const cols = 4, rows = 2;
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const x = baseX + c;
      const y = baseY + r;
      // light_nolabels → soft, map-only tiles, perfect for a monochrome base.
      tiles.push({
        x, y,
  ...
  return (
    <div className="gis-map mono" key={tick} aria-hidden="true">
      ...
      <svg viewBox="0 0 600 240" preserveAspectRatio="xMidYMid slice" className="gis-svg">
        ...
        {/* Candidate site parcel — dashed outline */}
        ...
        {/* Pin with pulsing accuracy ring */}
...
function PromptInputAnim() {
  ...
  const [phase, setPhase] = React.useState('typing'); // typing, thinking, results, hold
  const [typed, setTyped] = React.useState('');
  const [resultsTyped, setResultsTyped] = React.useState(['', '', '']);
  const [activeResult, setActiveResult] = React.useState(-1);

  // Type prompt → think → type results → hold → next
  React.useEffect(() => {
    let cancelled = false;
...
// Smooth-scroll to a hash target. Falls back to hard nav if the target
// isn't on this page (e.g. clicking a #section link from team.html).
function smoothScrollTo(hash, e) {
  if (!hash || hash === '#') return;
  const el = document.getElementById(hash.slice(1));
  if (!el) return;  // let the browser follow the link to index.html#xyz
...
function navClick(hash) {
  ...
  // Not on this page — let the anchor navigate to index.html#hash.
  return undefined;
}
...
// Animated typed number: reveals `value` char-by-char when `active` is true,
// after an initial `delay`. When `active` flips false, it resets so the next
// hover re-runs the type-in. Adds a subtle blinking caret while typing.
  ...
  React.useEffect(() => {
    ...
    const start = setTimeout(() => {
      ...
      const step = () => {
        ...
        if (i < value.length) {
          // Slight per-char jitter so it feels hand-typed, not mechanical.
          const base = 38;
...
function Hero() {
  ...
  return (
    <section className="hero">
      ...
      {/* Glass hover buttons anchored over scene features */}
      {properties.map(p => (
        <div
          key={p.key}
      ...
      <div className="hero-center">
        <div className="hero-center-mid">
          ...
          <div className="hero-cta-row">
            <a href="#" onClick={window.openDemoModal} className="btn btn-dark">Request a demo <span className="arrow">→</span><span className="brick-shadow" aria-hidden="true" /></a>
...
function TrustedBy_old_unused() {
  const logos = [
    { t: 'AON',        cls: 'tight' },
    { t: 'EPC Group',  cls: '' },
    { t: 'United Rentals', cls: 'mono' },
    { t: 'GM',         cls: 'tight' },
    { t: 'Molzer',     cls: 'serif' },
    { t: 'Hines',      cls: 'italic' },
  ];
  return (
    <section className="trusted" style={{display:'none'}}>
      <div className="container trusted-inner">
        <div className="trusted-label">Trusted by owners, developers &amp; GCs</div>
        <div className="trusted-logos">
          {logos.map(l => (
            <span key={l.t} className={'logo-chip ' + l.cls}>
    ...
    <section className="section" id="cta" style={{ paddingBottom: 80 }}>
      <div className="container">
        <div className="cta">
          ...
          <div className="row">
            <a href="#" onClick={window.openDemoModal} className="btn btn-primary">Request a demo <span className="arrow">→</span><span className="brick-shadow" aria-hidden="true" /></a>
            <a href="#" onClick={window.openDemoModal} className="btn btn-ghost">Talk to the team</a>
          </div>
        </div>
      </div>
    </section>
...
Object.assign(window, { Nav, Hero, TrustedBy, Steps, Bento, Verticals, CTA, Footer });
